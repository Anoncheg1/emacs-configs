...
2) Make shure that Emacs configured with “tree-sitter”
: M-: (treesit-available-p) RET ; should be t
or
: C-h v system-configuration-options RET

3) install markdown-ts-mode (json-ts-mode and yaml-ts-mode are build-in)
#+begin_src elisp :lexical t :results none :exports code :eval no
(require 'package)
...
4) Configure loading, file extensions and Org mode.
#+begin_src elisp :lexical t :results none :exports code :eval no
...
5) For ability to use “C-c C-c” ~org-ctrl-c-ctrl-c~ to parse and lint, ~org-babel-execute:yaml~ function required (commonly distributed in ~ob-yaml.el~).
...
: % m
: \.mp4
2) Invert (dired-toggle-marks)
: t
3) Kill “hide” files (dired-do-kill-lines)
: k
...
2) You can highlight cursor:
: (global-hl-line-mode 1)
: ;; underline the current line
: (set-face-attribute hl-line-face nil :underline t)

3) You can change background color:
#+begin_src elisp :results none :exports code :eval no
