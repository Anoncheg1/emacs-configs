...
#+end_src
* GUI: How to open file in frame with same major mode files?
This is required if you group buffers in frames by a major-mode.
#+begin_src elisp :results output :exports both
(defun my/find-file-frame(filename)
  "Open file in frame with the same mode buffer.
Used with `tab-line-mode' with (setopt tab-line-tabs-function
'tab-line-tabs-mode-buffers)."
    (find-file-noselect filename)
    (switch-to-buffer-other-frame filename))
#+end_src
